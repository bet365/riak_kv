...
do_update({vnode_index_write, PostingsAdded, PostingsRemoved}) ->
  update_stats([
    ...
    {[vnode, index, writes, postings], PostingsAdded},
    {[vnode, index, deletes, postings], PostingsRemoved}
  ]);
do_update({vnode_index_delete, Postings}) ->
  update_stats([
    {[vnode, index, deletes], Postings},
    {[vnode, index, deletes, postings], Postings}]);
...
do_update({get_fsm, Bucket, Microsecs, Stages, NumSiblings, ObjSize, PerBucket, undefined}) ->
  update_stats([
    {[node, gets], 1},
    {[node, gets, time], Microsecs},
    {[node, gets, siblings], NumSiblings},
    {[node, gets, objsize], ObjSize}]),
  ok = do_stages([node, gets, time], Stages),
...
do_update({consistent_get, _Bucket, Microsecs, undefined}) ->
  update_stats([
    {[consistent, gets], 1},
    {[consistent, gets, time], Microsecs}]);
...
do_update({consistent_put, _Bucket, Microsecs, undefined}) ->
  update_stats([
    {[consistent, puts], 1},
    {[consistent, puts, time], Microsecs}]);
...
do_update({write_once_put, Microsecs, ObjSize}) ->
  update_stats([
    {[write_once, puts], 1},
    {[write_once, puts, time], Microsecs}]),
